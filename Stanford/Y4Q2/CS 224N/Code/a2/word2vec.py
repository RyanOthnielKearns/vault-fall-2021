...
def negSamplingLossAndGradient(
    ...
    # components to do with u_o
    outsideWordVec = outsideVectors[outsideWordIdx]
    sig = sigmoid(outsideWordVec @ centerWordVec)
    loss = -np.log(sig)
    gradCenterVec = outsideWordVec * (sig - 1)
    gradOutsideVecs[outsideWordIdx] = centerWordVec * (sig - 1)

    # sum components for each u_{w_s}
    for i in negSampleWordIndices:
        sig = sigmoid(np.dot(outsideVectors[i], centerWordVec))
        loss -= np.log(1 - sig)
        gradCenterVec += outsideVectors[i] * sig
        gradOutsideVecs[i] += centerWordVec * sig
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for w in outsideWords:
        ...
        loss += loss_w
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterVec_w
