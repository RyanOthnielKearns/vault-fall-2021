...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S": self.stack.append(self.buffer.pop(0))
        elif transition == "LA":
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for i, parse in enumerate(minibatch): parse.parse_step(transitions[i])
        unfinished_parses = [parse for parse in unfinished_parses if len(parse.stack) != 1 or parse.buffer]
