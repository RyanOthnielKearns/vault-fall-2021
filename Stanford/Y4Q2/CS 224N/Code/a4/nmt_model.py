...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # X = torch.index_select(self.model_embeddings.source, 1, source_padded.flatten()) \
        #     .view(self.model_embeddings.embed_size, source_padded.shape[0], source_padded.shape[1]) \
        #     .permute((1, 2, 0))
        # assert(X.size == torch.size([source_padded.shape[0], source_padded.shape[1], self.model_embeddings.embed_size]))
